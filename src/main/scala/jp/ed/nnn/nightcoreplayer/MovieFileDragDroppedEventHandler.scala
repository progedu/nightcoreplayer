...
class MovieFileDragDroppedEventHandler(movies: ObservableList[Movie])
    extends EventHandler[DragEvent] {
    ...
    if (db.hasFiles) {
      db.getFiles.toArray(Array[File]()).toSeq.foreach { f =>
        ...
        player.setOnReady(() => {
          ...
          val movie =
            Movie(System.currentTimeMillis(), fileName, time, filePath, media)
