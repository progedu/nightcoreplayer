...
) extends TableCell[Movie, Long] {
  val hBox = new HBox()
  hBox.setAlignment(Pos.CENTER)
  val button = new Button("X")
  hBox.getChildren.addAll(button)

  override def updateItem(id: Long, empty: Boolean) = {
    ...
    if (empty) {
      ...
      button.setOnAction((event: ActionEvent) => {
        ...
        if (
          tableView.getSelectionModel.getSelectedItem != null && tableView.getSelectionModel.getSelectedItem.id == id
