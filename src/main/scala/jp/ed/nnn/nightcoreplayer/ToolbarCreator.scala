...
object ToolbarCreator {
  def create(mediaView: MediaView, tableView: TableView[Movie], timeLabel: Label, scene: Scene, primaryStage: Stage): HBox = {
    ...
    // stop button
    val stopButton = createButton("STOP.png", new EventHandler[ActionEvent]() {
      override def handle(event: ActionEvent): Unit = {
    ...
    toolBar.getChildren.addAll(
      firstButton, backButton, playButton, pauseButton, stopButton, forwardButton, lastButton, fullscreenButton, timeLabel)
    toolBar
