...
object ToolbarCreator {
  ...
  private[this] def back(mediaView: MediaView): Unit = {
  ...
  private[this] def play(mediaView: MediaView, tableView: TableView[Movie]): Unit ={
  ...
  private[this] def pause(mediaView: MediaView){
    if (mediaView.getMediaPlayer != null) {
      mediaView.getMediaPlayer.pause()
    }
  }

  private[this] def forward(mediaView: MediaView) = {
    if (mediaView.getMediaPlayer != null) {
  ...
  def create(mediaView: MediaView, tableView: TableView[Movie], timeLabel: Label, scene: Scene, primaryStage: Stage): HBox = {
    ...
    val backButton = createButton("back.png", new EventHandler[ActionEvent]() {
      override def handle(event: ActionEvent): Unit = back(mediaView)
    })

    // play button
    val playButton = createButton("play.png", new EventHandler[ActionEvent]() {
      override def handle(event: ActionEvent): Unit = play(mediaView, tableView)
    })

    // pause button
    val pauseButton = createButton("pause.png", new EventHandler[ActionEvent]() {
      override def handle(event: ActionEvent): Unit = pause(mediaView)
    })

    // forward button
    val forwardButton = createButton("forward.png", new EventHandler[ActionEvent]() {
      override def handle(event: ActionEvent): Unit = forward(mediaView)
    })
    ...
    scene.setOnKeyPressed(new EventHandler [KeyEvent] () {
      override def handle (event: KeyEvent) = {
        event.getCode match {
          case KeyCode.H => back(mediaView)
          case KeyCode.J => play(mediaView, tableView)
          case KeyCode.K => pause(mediaView)
          case KeyCode.L => forward(mediaView)
          case _         => // System.out.println(event.getCode)
        }
      }
    })
