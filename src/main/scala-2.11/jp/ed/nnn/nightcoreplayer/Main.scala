...
class Main extends Application {
  ...
  override def start(primaryStage: Stage): Unit = {
    ...
    val volSlider = ToolBarButtonCreator.createVolumeSlider(mediaView)
        ...
        row.setOnMouseClicked(new EventHandler[MouseEvent] {
          override def handle(event: MouseEvent): Unit = {
            if (event.getClickCount >= 1 && !row.isEmpty) {
              MoviePlayer.play(row.getItem, tableView, mediaView, timeLabel, rateLabel, volSlider)
            }
          }
        })
        row
    ...
    val toolBar = ToolBarCreator.create(scene, mediaView, tableView, timeLabel,
      primaryStage, baseBorderPane, volSlider, rateLabel)
