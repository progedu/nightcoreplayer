  ...
  override def start(primaryStage: Stage): Unit = {
    ...
    primaryStage.fullScreenProperty().addListener(new ChangeListener[java.lang.Boolean]{
      override def changed(observable: ObservableValue[_ <: lang.Boolean], oldValue: lang.Boolean, newValue: lang.Boolean): Unit = {
        toolBar.setVisible(oldValue)
        toolBar.setManaged(oldValue)
        tableView.setVisible(oldValue)
        tableView.setManaged(oldValue)
        var mul: Int = 1
        if (oldValue)
          mul = 1
        else
        ...
        mediaView.fitWidthProperty().bind(scene.widthProperty().subtract(tableMinWidth*mul))
        mediaView.fitHeightProperty().bind(scene.heightProperty().subtract(toolBarMinHeight*mul))
      }
    })
    primaryStage.show()
  }
